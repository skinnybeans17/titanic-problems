...
const getSurvivorCountForClass = (data, pclass) => {
	const survivors = data.filter(item => item.fields.survived === "Yes" && item.fields.pclass === pclass);
	return survivors.length
}
...
const getCasualityCountForClass = (data, pclass) => {
	const casualties = data.filter(item => item.fields.survived === "No" && item.fields.pclass === pclass);
	return casualties.length
}
...
const getMinAge = (data) => {
	...
	const miniumAge = Math.min(...validAges.map(item => item.fields.age));
	return miniumAge
}
...
const getMaxAge = (data) => {
	const validAges = data.filter(item => item.fields.age !== undefined);
	const maxiumAge = Math.max(...validAges.map(item => item.fields.age));
	return maxiumAge
}
...
const getMinFare = (data) => {
	...
	const miniumFare = Math.min(...validFares.map(item => item.fields.fare));
	return miniumFare
}
...
const getMaxFare = (data) => {
	...
	const maxiumFare = Math.max(...validFares.map(item => item.fields.fare));
	return maxiumFare
}
...
const getPassengersByGender = (data, gender) => {
	const passenegers = data.filter(item => item.fields.sex === gender);
...
const getSurvivorsByGender = (data, gender) => {
	const survivors = data.filter(item => item.fields.survived === "Yes" && item.fields.sex === gender);
	return survivors.length
}
...
const getCasualitiesByGender = (data, gender) => {
	const casualties = data.filter(item => item.fields.survived === "No" && item.fields.sex === gender);
	return casualties.length
}
...
const getMedianFare = (data) => {
	...
	if (length % 2 === 0) {
		const sumValuesMiddle = sortedFares[indexMiddle - 1] + sortedFares[indexMiddle];
		return sumValuesMiddle / 2;
	} else {
		return sortedFares[indexMiddle]
	}
}
...
const getAverageAge = (data) => {
	const validAges = data.filter(item => item.fields.age !== undefined);
	const totalAge = validAges.reduce((sum, item) => item.fields.age + sum, 0);
...
const getMedianAge = (data) => {
	...
	const indexMiddle = Math.floor(length / 2);
	if (length % 2 === 0) {
		const sumValuesMiddle = sortedAges[indexMiddle - 1] + sortedAges[indexMiddle];
		return sumValuesMiddle / 2;
	} else {
		return sortedAges[indexMiddle]
	}
}
